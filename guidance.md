...
### FASE 0 - Authentication
| Step | Action | Detail |
| --- | --- | --- |
| 0.1 | Register akun | User daftar dengan email + password (min 8 karakter). Backend simpan `password_hash`, bukan password plaintext |
| 0.2 | Login | User login via email + password, backend return `access_token` |
| 0.3 | Persist session | Client simpan token aman di storage platform, kirim `Authorization: Bearer <token>` untuk endpoint user |
| 0.4 | Scope enforcement | Semua endpoint `/api/users/{user_id}/...` wajib cocok dengan `user_id` di token |
...
CREATE TABLE users (
  ...
  password_hash   VARCHAR(255) NOT NULL COMMENT 'Hash only, generated by backend auth service',
...
| # | Issue | Perubahan |
| --- | --- | --- |
| A1 | Belum ada login user | Ditambahkan flow auth: register, login, token session di App Flow Fase 0 |
| A2 | Data bisa diakses lintas user | Semua endpoint `/api/users/{user_id}/...` wajib validasi owner berdasarkan token |
| A3 | Tabel user belum siap auth | Kolom `password_hash` ditambahkan ke tabel `users` (hash only, tanpa plaintext) |
| A4 | Dokumentasi API belum mencerminkan auth | Dokumentasi endpoint auth + aturan Authorization diperbarui |
